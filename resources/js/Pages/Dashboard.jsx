        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] dark:bg-[radial-gradient(#18181b_1px,transparent_1px)] [background-size:16px_16px] font-sans selection:bg-gojek-100 selection:text-gojek-900 dark:selection:bg-gojek-900 dark:selection:text-gojek-100 overflow-hidden h-screen flex flex-col relative z-0">
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
                                ...
                                <div className="grid grid-cols-2 sm:flex sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                                    ...
                                    >
                                        ...
                                        <span>{isSyncing ? 'Updating...' : 'Update Harga'}</span>
                                    </button>

                                    {auth.user && ((summary?.totalFloatingProfit || 0) + (summary?.totalNetProfit || 0)) > 0 && (
                                        <button
                                            onClick={() => setIsFlexModalOpen(true)}
                                            className="w-full col-span-1 sm:w-auto whitespace-nowrap inline-flex items-center justify-center px-3 py-2.5 sm:py-2 text-[13px] sm:text-sm font-semibold text-white transition-all duration-200 bg-gradient-to-r from-emerald-500 to-teal-600 border border-transparent rounded-lg hover:from-emerald-600 hover:to-teal-700 shadow-sm focus:outline-none"
                                        >
                                    ...
                                    {auth.user ? (
                                        <button
                                            ...
                                            className="w-full col-span-2 sm:col-span-1 sm:w-auto whitespace-nowrap inline-flex items-center justify-center px-3 py-2.5 sm:py-2 text-[13px] sm:text-sm font-semibold text-white transition-all duration-200 bg-gojek-500 border border-transparent rounded-lg hover:bg-gojek-600 shadow-sm focus:outline-none"
                                        >
                                                                    ...
                                                                    <tr key={trx.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors">
                                                                        ...
                                                                        <td className="px-6 py-4 whitespace-nowrap text-right">
                                                                            <div className="flex items-center justify-end space-x-1">
                                                                                {trx.status === 'open' && (
                                                                                    <button onClick={() => openSellModal(trx)} className="flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-900/30 hover:bg-emerald-200 dark:hover:bg-emerald-900/50 rounded-lg transition-colors border border-emerald-200 dark:border-emerald-800/50 shadow-sm">
                                                                                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                                                        <span>Jual</span>
                                                                                    </button>
                                                                                )}
                                                                                <button onClick={() => openEditTrx(trx)} className="flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-lg transition-colors border border-zinc-200 dark:border-zinc-700 shadow-sm">
                                                                                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                                                                    <span>Edit</span>
                                                                                </button>
                                                                                <button onClick={() => openDeleteTrx(trx)} className="flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold text-rose-700 dark:text-rose-400 bg-rose-100 dark:bg-rose-900/30 hover:bg-rose-200 dark:hover:bg-rose-900/50 rounded-lg transition-colors border border-rose-200 dark:border-rose-800/50 shadow-sm">
                    ...
                    activeStocks={(() => {
                        ...
                        return Object.keys(stockMap)
                            .map(code => {
                                ...
                                return { code, percentage: pct, logo: getStockLogo(code), profit: data.profit };
                            })
            ...
            <Modal show={isFlexModalOpen} onClose={() => setIsFlexModalOpen(false)} maxWidth="sm">
                <div className="p-6">
                    ...
                    <div className="w-full flex justify-center mb-6 bg-zinc-100/50 dark:bg-[#0A0A0A]/50 rounded-2xl p-4 overflow-hidden border border-zinc-200/50 dark:border-zinc-800/50 relative h-[250px] sm:h-[300px]">
                        {isGeneratingFlex ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-zinc-500 dark:text-zinc-400 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm z-10">
                                <svg className="animate-spin h-8 w-8 mb-3 text-emerald-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                <span className="text-xs font-bold tracking-widest uppercase">Memproses Gambar...</span>
                            </div>
                        ) : null}
                        
                        {flexImageUrl ? (
                            <img src={flexImageUrl} alt="Cuan Pamer" className="w-auto h-full max-h-[220px] sm:max-h-[270px] object-contain rounded-xl shadow-2xl border border-zinc-200/20" />
                        ) : !isGeneratingFlex ? (
