                                                ...
                                                {day && hasEvents && (
                                                    <div className="flex -space-x-1">
                                                        {uniqueIpos.slice(0, 3).map(({ ipo }) => (
                                                            <img key={ipo.id} src={`https://e-ipo.co.id/id/pipeline/get-logo?id=${ipo.id}`} alt="Logo" className="w-3.5 h-3.5 rounded-full border border-white dark:border-zinc-800 bg-white object-contain shadow-sm" onError={(e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${ipo.ticker?.substring(0, 2)}&background=000&color=fff`; }} />
                                                        ))}
